...
import { View, Text, StyleSheet, ScrollView, Alert, SectionList,StatusBar } from 'react-native';
...
const HomeScreen = () => {
      ...
      const phoneNumber = await AsyncStorage.getItem('logined');
        try {
          ...
          const updatedData = data.map((view,level) => ({
            ...
            lessons: view.lessons.map((lesson, index) => ({
              ...lesson,
              auth: (purchased1 || isAdmin || level==0 || (level==1 && (index==0 || index ==1))) ? true : false ,
    ...
    const sections = viewsData?.map((level, index) => ({
      ...
      subTitle:level.lesson_subtitle,
      data: level.lessons,
      key: index,
    }));
    return (
        ...
        loading?<LoadingScreen/>:
            <View style={styles.container}>
                <View style={styles.header}>
                    <Header isAdmin={isAdmin} style={{zIndex:4}}/>
                </View>
                {/* <ScrollView> */}
                  <SectionList
                  style={{zIndex:1}}
                    ...
                    renderItem={({ item, index }) => (
                      <LessonComponent data={item} key={index} index={index} pos={index} loading={loading} setLoading={setLoading} />
                    )}
                    renderSectionHeader={({ section,key }) => (
                      <LevelComponents data={section} key={section.key} index={section.key} progress={progressArray[section.key]}/>
                    )}
