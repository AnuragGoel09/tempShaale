...
import { View, Text, StyleSheet, ScrollView, Alert, TouchableOpacity, FlatList, BackHandler, TextInput, Dimensions } from 'react-native';
import { Card, Layout, Button, Modal, Select, SelectItem, Toggle } from '@ui-kitten/components';
...
const LessonDetails = ({ route, navigation }) => {
  ...
  const moveLessonToView = async () => {
    if (!selectedView) {  // Handle case when no view is selected
      return;
    }
    try {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={[
        styles.viewItem,
        selectedView && selectedView.lesson_id === item.lesson_id ? styles.selectedItem : null,
      ]}
    ...
    >
      <Text
        style={[
          styles.itemText,
          selectedView && selectedView.lesson_id === item.lesson_id ? styles.selectedItemText : null,
        ]}
      >{item.lesson_title}</Text>
    </TouchableOpacity>
  );
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.infoContainer}>
        ...
        <Text style={styles.headerDisplay}>ID: <Text style={styles.info}>{lesson.id}</Text></Text>
        <Text style={styles.headerDisplay}>Title: <Text style={styles.info}>{lesson.Title}</Text></Text>
        <Text style={styles.headerDisplay}>Tempo: <Text style={styles.info}>{lesson.Tempo}</Text></Text>
        {/* <Text style={styles.headerDisplay}>Time Cycle: <Text style={styles.info}>{lesson.TimeCycle}</Text></Text> */}
        <Text style={styles.headerDisplay}>Beats: <Text style={styles.info}>{lesson.Beats}</Text></Text>
        ...
        <Text style={styles.headerDisplay}>Total Beats: <Text style={styles.info}>{lesson.totalBeats}</Text></Text>
        {/* <Text style={styles.headerDisplay}>Tags: <Text style={styles.info}>{lesson.Tags.join(', ')}</Text></Text> */}
        <Text style={styles.headerDisplay}>
          Timestamp: <Text style={styles.info}>{new Date(lesson.timestamp._seconds * 1000).toLocaleString()}</Text>
        </Text>
      ...
      <Button
        ...
        onPress={() => {
          navigation.navigate('AddLyrics', { lessonId: lesson.id, isStandard: false });
        }}
      >
        Add Lyrics
      </Button>
      ...
      <View style={styles.metronomeToggleContainer}>
        <Toggle
          ...
          onChange={muteSound}
        >
          <Text style={styles.metronomeLabel}>{!isMuted ? 'Metronome ON' : 'Metronome OFF'}</Text>
        </Toggle>
      </View>
      ...
      >
        <View style={[styles.modalContainer, { width: screenWidth * 0.75 }]}>
          <View style={styles.modalContent}>
            ...
            />
            <Button style={styles.modalButton} onPress={moveLessonToView}>Move Lesson</Button>
            <Button style={styles.modalButton} onPress={() => setModalVisible(false)}>Cancel</Button>
          </View>
        </View>
      </Modal>
